...
def get_exchangers_rates(html, fields, carrency):
    ...
    for row in rows_exchagers:
        exchangers_table.append({
            ...
            fields[2] : row.find('div', attrs={'data-currencies-code' : carrency})['data-currencies-rate-buy'],
            fields[3] : row.findAll('div', attrs={'data-currencies-code' : carrency})[1]['data-currencies-rate-sell'],
            'update_time' : row.find('div', class_='font-size-small color-gray-gray').text[:-1]
        })

    return exchangers_table

def get_banks_rates(html, fields, carrency):
    soup = BeautifulSoup(html, 'html.parser')
    ...
    for row in rows_banks:
        banks_table.append({
            ...
            fields[1] : row.find('td', attrs={'data-currencies-code' : carrency})['data-currencies-rate-buy'],
            fields[2] : row.findAll('td', attrs={'data-currencies-code' : carrency})[1]['data-currencies-rate-sell'],
...
def save_csv(result_table, save_file, fields):
    with open(save_file, "w", encoding="utf8", newline='') as csvf:
